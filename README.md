...
截至 v1.0.2，下面这些应用已经接入真实后端、服务器文件系统、服务器网络请求或服务器侧持久化：

- **系统与桌面**：登录门、文件管理器、终端、废纸篓、系统监视器、任务管理器、审计日志、设置、内置浏览器。
- **文件与网络工具**：文本编辑器、Markdown 编辑器、图片查看器、下载管理器、API 测试器、RSS 阅读器、Git 客户端、SSH 客户端、FTP/SFTP 客户端、书签、天气、邮件客户端。
- **个人数据应用**：聊天应用、便签、日历、笔记本、电子表格、思维导图、演示文稿、番茄钟、习惯追踪、词典、翻译。它们通过 `/api/app-state` 或专用 API 存在服务器侧，浏览器本地状态只作为迁移兜底。
- **媒体与文档应用**：相册、相机、录音机、音乐播放器、视频播放器、PDF 阅读器、绘图。媒体文件写入服务器用户主目录下的 `~/.mochan/media/<kind>`，再通过 `/api/fs/download` 读取。
- **低优先级同步项**：计算器历史、Snake 最高分、2048 最高分、电台收藏和偏好已接入 `/api/app-state`。

没有具体保存需求的时钟、取色器、Base64 工具、二维码生成器、密码生成器、正则测试器、JSON 编辑器，以及棋类 / 小游戏 / 白噪音 / 节拍器仍保持前端运行；只有出现跨设备同步、文件保存、排行榜、预设或历史记录需求时再补专用后端。v1.0.2 的完整收尾说明见 [`docs/release-notes-v1.0.2.md`](docs/release-notes-v1.0.2.md)。
...
| 桌面 + StatusBar + Dock | 应用启动器 |
|---|---|
| ![桌面 + StatusBar + Dock](docs/img/desktop.png) | ![应用启动器](docs/img/app-launcher.png) |
| **文件管理器** | **终端** |
| ![文件管理器](docs/img/file-manager.png) | ![终端](docs/img/terminal.png) |
| **设置：主题与壁纸** | **内置浏览器起始页** |
| ![设置：主题与壁纸](docs/img/settings.png) | ![内置浏览器起始页](docs/img/browser.png) |
