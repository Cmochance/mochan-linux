...
截至 v1.0.1，下面这些应用已经接入真实后端、服务器文件系统、服务器网络请求或服务器侧持久化：
...
- **个人数据应用**：聊天应用、便签、日历、笔记本。它们通过 `/api/app-state` 或专用 API 存在服务器侧，浏览器本地状态只作为迁移兜底。

正在补充专用后端能力的应用包括：电子表格、思维导图、演示文稿、番茄钟、习惯追踪、词典、翻译、相册、相机、录音机、音乐播放器、视频播放器、PDF 阅读器、绘图。

计算器、时钟、取色器、Base64 工具、二维码生成器、密码生成器、正则测试器、JSON 编辑器，以及棋类 / 小游戏 / 白噪音 / 节拍器暂时不优先做独立后端；只有出现跨设备同步、文件保存、排行榜、预设或历史记录需求时再补。完整排序和执行状态见 [`app-backend-completion-task.md`](app-backend-completion-task.md)。
